...
<div class="modal-dialog modal-xl">
    <div class="modal-content">
      ...
      <form action="{{route('admin.areas.update',[$item->id])}}" method="POST" enctype="multipart/form-data" id="ajax_form">
                    @csrf
                    @method('PATCH')
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="area_name" class="form-control" name="area_name" placeholder="Area Name..." value="{{ $item->area_name }}">
                            </div>
                        </div>  
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select form-select-sm" name="city_id">
                                    @foreach($cities as $city)
                                        <option value="{{ $city->id }}" {{ $city->id == $item->city_id ? 'selected' : '' }} >{{ $city->city }}</opiton>   
                                    @endforeach                                 
                                </select>
                            </div>
                        </div>  
